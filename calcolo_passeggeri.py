...
def calcolo_passeggeri_per_singola_zona(data, bor, keys):
    ...
    identifier = genera_identificatore_fascia(keys)
    data = data[data['Borough']==bor]
    for i in range(len(keys)):
        contatore = 0
        indici = []
        for j in range(identifier[i][0],identifier[i][1]):
            #verifico la fascia oraria e controllo, tramite la lista "indici" che la corsa non sia già stata considerata precedentemente
