...
def verifica_numero_mesi(anno, mesi):
    ...
    for i in anno:
        if (int(i) == 2022):
            if (len(mesi) > 10):
                raise Exception(f'Per il 2022 non sono disponibili {len(mesi)} mesi. Sceglierne massimo 10.')
            for m in ['11', '12']:
                if m in mesi:
                    raise Exception(f'Per il 2022 non è disponibile il mese {m}. Scegliere massimo fino a 10.')
